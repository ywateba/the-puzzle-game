...
class NumericPuzzleSolver:
    ...
    def __valid_results(self):
        return [
            result for result in self.results if result["result"] == self.puzzle.target
        ]
    ...
    def __reach_target(self, entries, path):
        if len(entries) == 1:
            ...
            if self.__is_close(result):
                self.results.append({"result": entries[0], "operations": path})
        else:
                    ...
                    for operation in Operation.operations():
                        ...
                        if result >= 0:
                            if result == self.puzzle.target:
                                path.append(op_name)
                                self.results.append(
                                    {"result": result, "operations": path}
                                    # NumericPuzzleSolution(result,path)
                                )
                            else:
    ...
    def solve(self):
        ...
        self.puzzle.solutions = [NumericPuzzleSolution(**result) for result in results]
